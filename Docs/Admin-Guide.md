...
Draft invoices are written to `Documents\Molehill Admin\Invoices\`. Open one in a browser and print it to PDF (A4, with the print margins already set).

**How an invoice looks.** It follows the Molehill. dataServices style guide: the logo at the top left, your details on the right, a cyan rule, then bill-to / from / what it is for, the lines, the totals, your bank details and the terms. Colours are the brand's three - `#231F20`, `#44C8F5` and white - and headings use Krungthep where it is installed, falling back to Bahnschrift on Windows. The logo is stored in `dbo.Setting` as `LogoDataUri`, so each invoice is a single self-contained file; change it in **Molehill Manager > File > Business and invoice details > Logo file** (a PNG about 500 px wide), or clear it to print the business name instead. `BusinessTradingName`, `BusinessContact` and `BusinessPhone` fill in the rest of the header. A cycle with nothing covered and nothing owed gets no invoice, and the billing run tells you so. That usually means an instance's covered-from date is wrong.
